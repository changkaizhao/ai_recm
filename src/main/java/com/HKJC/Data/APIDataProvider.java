...
public class APIDataProvider extends DataProvider{
    ...
    public APIDataProvider(String mtg_id, String acc_id, String messageID, int race_no) throws Exception{
        try{
            ...
            String racePool = DataProvider.HttpGet(Configurator.getInstance().api_racepool, meeting_id, null, messageID);
            ...
            String raceTimeData = DataProvider.HttpGet(Configurator.getInstance().api_racetime, meeting_id, null, messageID);
            ...
            String winOddsData = DataProvider.HttpGet(Configurator.getInstance().api_winodds, meeting_id, null, messageID);
            ...
            String cwaGroupData = DataProvider.HttpGet(Configurator.getInstance().api_cwagroup, meeting_id, null, messageID);
            this.cwaGroup = Arrays.asList(objectMapper.readValue(cwaGroupData,CWAGroup[].class));
//            if(this.cwaGroup == null || this.cwaGroup.size() == 0){
//                throw  new HKJCException(501, "No cwaGroups get from " + Configurator.getInstance().api_cwagroup+ " meetingID:" + meeting_id);
//            }

            String runnersData = DataProvider.HttpGet(Configurator.getInstance().api_runner, meeting_id, null, messageID);
