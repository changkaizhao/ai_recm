...
public class FileDataProvider extends DataProvider {
    ...
    private void loadDataAsync(ObjectMapper objectMapper, String messageID, String acc_id, boolean useProxi)
            throws Exception {
        ...
        Callable<Void> raceProximityDataTask = () -> {
            try {
                long start = System.nanoTime();
                // String raceProximityData =
                // DataProvider.HttpGet(Configurator.getInstance().api_raceProximity,
                // meeting_id, acc_id, messageID);
                // float end = (System.nanoTime() - start)/(float)1000000.0;
                // logger.info("[MessageID]:"+messageID + " ⏰[FetchingRaceProximityData]:" + end
                // + "ms");
                ...
                // this.raceOdds =
                // Arrays.asList(objectMapper.readValue(raceProximityData,RaceOdd[].class));
        ...
        Callable<Void> bettypeDataTask = () -> {
            try {
                long start = System.nanoTime();
                // String bettypeData =
                // DataProvider.HttpGet(Configurator.getInstance().api_bettype, meeting_id,
                // acc_id, messageID);
        ...
        Callable<Void> horseDataTask = () -> {
            try {
                long start = System.nanoTime();
                // String horseData = DataProvider.HttpGet(Configurator.getInstance().api_horse,
                // meeting_id, acc_id, messageID);
    ...
    private void asyncFilterObjByRaceNo(String messageID, ObjectMapper objectMapper) throws Exception {
        ...
        Callable<Void> filterHorse = () -> {
            try {
                ...
                // start = System.nanoTime();
                // this.horseOdds =
                // Arrays.asList(objectMapper.readValue(horseData.toString(),HorseOdd[].class));
        ...
        Callable<Void> filterBettype = () -> {
            try {
                ...
                // start = System.nanoTime();
                // this.betTypeOdds =
                // Arrays.asList(objectMapper.readValue(bettypeData.toString(),
                // BetTypeOdd[].class));
    ...
    public FileDataProvider(String mtg_id, String acc_id, String messageID, int race_no) throws Exception {
        ...
        if (this.racepool == null || this.racepool.size() == 0) {
            throw new HKJCException(501,
                    "No race status get from " + Configurator.getInstance().api_racepool + " meetingID:" + meeting_id);
        }
        if (this.raceTimes == null || this.raceTimes.size() == 0) {
            throw new HKJCException(501,
                    "No raceTimes get from " + Configurator.getInstance().api_racetime + " meetingID:" + meeting_id);
        }
        ...
        if (this.runners == null || this.runners.size() == 0) {
            throw new HKJCException(501,
                    "No runners get from " + Configurator.getInstance().api_runner + " meetingID:" + meeting_id);
        }
        ...
        // throw new HKJCException(501, "cannot get horse and bettype data from api " +
        ...
        if (this.winOdds == null || this.winOdds.size() == 0) {
            throw new HKJCException(501,
                    "No winOdds after filter with raceNo: " + this.race_no + " meetingID:" + meeting_id);
        }
        if (this.runners == null || this.runners.size() == 0) {
            throw new HKJCException(501,
        ...
        if (this.horseOdds == null || this.horseOdds.size() == 0) {
            throw new HKJCException(501, "No horseOdds get from " + Configurator.getInstance().api_horse + " meetingID:"
                    + meeting_id + " betAcc:" + acc_id);
        }
